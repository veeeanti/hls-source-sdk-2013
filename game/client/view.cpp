...
void CViewRender::OnRenderStart()
{
	...
	if ( player )
	{
		...
		int iDefaultFOV = default_fov.GetFloat();
...
void CViewRender::Render( vrect_t *rect )
{
    ...
    for( StereoEye_t eEye = GetFirstEye(); eEye <= GetLastEye(); eEye = (StereoEye_t)(eEye+1) )
	{
	    ...
	    {
		    limitedAspectRatio = MIN( aspectRatio, 1.85f * 0.90f ); // cap out the FOV advantage at a 1.85:1 ratio (about the widest any legit user should be)
	    }
